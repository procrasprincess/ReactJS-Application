...
The [Readable stream][2] object returned by the above request methods emits the following events, in addition to the regular ones (e.g. `end`, `close`, `data`, `pipe`, `readable`).

### Event: `'response'`

 - `response <http.IncomingMessage>`

Emitted when the underlying [http.ClientRequest][3] emits a response event. This is after the connection is established and the header received, but before any of it is processed (e.g. authorization required or redirect to be followed). No data has been consumed at this point.

### Event: `'redirect'`

 - `location <String>
	`
Indicates that the a redirect is being followed. This means that the response code was a redirect (`301`, `302`, `303`, `307`) and the given [redirect options][4] allowed following the URL received in the `Location` header.
...
For information about options that've changed, there's always [the changelog][5].

 - `agent`       : Uses an [http.Agent][6] of your choice, instead of the global, default one. Useful for tweaking the behaviour at the connection level, such as when doing tunneling (see below for an example).
...
These options are passed directly to `https.request` if present. Taken from the [original documentation][7]:
...
Since you can pass a custom HTTPAgent to Needle you can do all sorts of neat stuff. For example, if you want to use the [`tunnel`][8] module for HTTPS proxying, you can do this:
...
Or use [RFC-1738][9] basic auth URL syntax:
...
[1]:	https://nodei.co/npm/needle/
[2]:	https://nodejs.org/api/stream.html#stream_class_stream_readable
[3]:	https://nodejs.org/api/http.html#http_class_http_clientrequest
[4]:	#redirect-options
[5]:	https://github.com/tomas/needle/releases
[6]:	https://nodejs.org/api/http.html#http_class_http_agent
[7]:	http://nodejs.org/docs/latest/api/https.html
[8]:	https://github.com/koichik/node-tunnel
[9]:	http://tools.ietf.org/html/rfc1738#section-3.1

[image-1]:	https://nodei.co/npm/needle.png